...
class Parser:
    ...
    def _fill_empty_terminals(self, dich: Dichotomy) -> bool:
        ...
        # Dicts here should have keys of previous rather than current reps?
        to_fill = [
        ...
        for filling in to_fill:
            ...
            fit = self._fit_element(neut, term_only=True)
            if not fit:
                return False
            else:
    ...
    def _decide_dichotomy(
        ...
        # Otherwise determine the fit the normal way
        ...
        if not fit:
            ...
            # If all fails
                ...
                print(f"""Couldn't match {e} to {dich}, masks {dich.masks[0]}-{dich.masks[1]}, 
                    pos={dich.masks[1].pos}, rep={dich.masks[1].rep}
                    """)
                return False
    ...
    def _close_clause(self) -> bool:
        ...
        validation = self._validate_mapping(elems)
